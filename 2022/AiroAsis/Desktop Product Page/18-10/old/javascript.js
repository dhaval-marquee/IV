function loadcode() {
    if (!$('body').hasClass('iv_desktop')) {
        ...
        $('.iv_desktop h3.product__block').text('Removes up to 99% of airborne allergens, dust, pollen, mold, and odors');

        function changePriceDesign() {
            $('.iv_desktop .main-product__price-net').before($('.main-product__original-price'));

            if($('.iv_desktop .main-product__price-difference').length > 0) {
        ...
        // Rearrange sections
        $($('.richtext').eq(0).closest('.shopify-section')).after($('.two-column-rich__content.wysiwyg').eq(0).closest('.shopify-section'));
