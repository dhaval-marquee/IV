function IVTestCode() {
    if (!$('body').hasClass('cybergrx_iv')) {
        ...
        $('.cybergrx_iv .content-banner').append($('.f-landing-right .style-ul-check ul'));
        $('.cybergrx_iv .mod-banner.mod-banner-l .content-banner ul').before('<p>Only 16% of organizations say they effectively manage third-party risks.</p>');
        $('.cybergrx_iv .mod-banner.mod-banner-l .content-banner ul').wrap('<div></div>');
        $('.cybergrx_iv .mod-banner.mod-banner-l .content-banner div').prepend('<p>Join us to learn:</p>');
