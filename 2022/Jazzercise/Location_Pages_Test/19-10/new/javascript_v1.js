function testCode() {
    if (!$('body').hasClass('location_iv')) {
        ...
        $('.hero_section').after($('.facility .price-container').parent().addClass('membership_sections').removeClass('gray-wrap'));
...
try {
    ...
    var checkHashValue = window.location.href.split('#')[1];

    if(checkHashValue != null && checkHashValue != undefined && checkHashValue != '') {
        $('html, body').animate({
            scrollTop: $("#"+checkHashValue).offset().top - 50
        }, 100);
    }
