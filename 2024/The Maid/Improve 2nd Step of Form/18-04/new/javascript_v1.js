function loadTestCode() {
    if (!document.querySelector('body').classList.contains('maids-form')) {
        ...
        observerElm(targetElement, function (mutationsList, observer) {
            updateLabels();
            zipCode(observer); // Pass the observer instance to the zipCode function
        });
        
        // Zipcode 
        function zipCode(observer) { 
            var debounceTimer;
            ...
            document.querySelector('#input_14_35').addEventListener('keyup', function (e) {
                debounce(function() {
                    var el = document.getElementById("input_14_35");
                    if (el.value && /^\d{5}$/.test(el.value)) {
                        var xhr = new XMLHttpRequest();
                        xhr.open("GET", "https://zip.getziptastic.com/v2/US/" + el.value, true);
                        xhr.onreadystatechange = function () {
                            if (xhr.readyState == 4) {
                                if (xhr.status == 200) {
                                    ...
                                    observer.disconnect(); // Disconnect observer after handling the error
                                }
                            }
                        };
                        xhr.send();
