function loadTestCode() {
    if (!document.querySelector('body').classList.contains('iv-faq-tab')) {
        ...
        // While on the "FAQ" tab, the FAQ will be shown twice and that's okay.
        if(ifFaq){
            ...
            // Move the whole .iv-datalist-faq div into
            const faqContainer = document.querySelector('#faq > .row');
            if (faqContainer && !faqContainer.contains(currentFaq)) {
                faqContainer.appendChild(currentFaq);
                currentFaq.classList.replace('col-lg-8', 'col-lg-12');
            }
