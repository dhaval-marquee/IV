...
function loadcode() {
    ...
    var collapse_page = function(event, ui) {
        if($(window).innerWidth() > 600) {
            var currFormHeight = Math.max($('.lp-pom-form .form_columns').height() - 80);
        } else {
    ...
    if(!$('body').hasClass('form_iv')) {
        ...
        if ($('#select_a_preferred_location option[value="No Preference"]').length > 0) {
           $('#select_a_preferred_location option[value=""]').after($('#select_a_preferred_location option[value="No Preference"]').eq(0));
        } else {
