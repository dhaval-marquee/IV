function loadTestCode() {
    ...
    $(window).on('click', function(event) {
        const $modal = $('.mq_modal');
        if ($modal.is(':visible') && !$(event.target).closest('.mq_modal_content').length && !$(event.target).hasClass('openModal')) {
            $('html').removeClass('mq_modal_open');
            $('body').removeClass('open');
            setTimeout(() => {
                $modal.hide();
            }, 75);
            $('.locationsSearchContainer').append($('#dynamicForm > form'));
        }
    });
