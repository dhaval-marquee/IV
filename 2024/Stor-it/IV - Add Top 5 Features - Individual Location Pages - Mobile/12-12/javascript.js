function loadTestCode() {
    if (!jQuery('body').hasClass('location_iv')) {
        ...
        var getNumberInterval = setInterval(function() {
            var locationFacilityInfo = document.querySelectorAll('#location_theme_1 #tab1 .six > p')[3];
            
            if (locationFacilityInfo) {
                locationFacilityInfo.insertAdjacentHTML('afterend', `<div class="features_title facility-title">Top Features:</div>`);
                const jsonData = {
                    product1: {
