function loadTestCode() {
	...
	if (!bodyEle.classList.contains('hero_iv')) {
        ...
        var getBgClass = setInterval(() => {
            if(jQuery('.hero_iv .hero_iv_slideone_title').closest('.swiper-slide-inner').prev().hasClass('swiper-slide-bg')) {
                clearInterval(getBgClass);
                jQuery('.hero_iv .hero_iv_slideone_title').closest('.swiper-slide-inner').prev().addClass('slide1_bg');
            }
        }, 100);
