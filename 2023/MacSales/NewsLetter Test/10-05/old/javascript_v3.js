function loadTestCode() {
    ...
    if (!bodyEle.classList.contains('newsLetter_Test')) {
        ...
        $('.newsLetter_Test .hero-builder__heading').text('Exclusive Access');


        if (!bodyEle.classList.contains('newsLetter_Test_sw')) {
            var icon = $('.newsLetter_Test #navigation-email a > span').detach()
            $('.newsLetter_Test #navigation-email a').text('Exclusive Access');
            ...
            $('.newsLetter_Test #navigation-mobile-upgrades').before(`<li id="navigation-mobile-upgrades" class="nav-mobile__link nav-mobile__link-icon">
                <a href="/shop/macsales_newsletter" aria-label="upgrades"><span class="nav-mobile__lower-icon"><svg fill="#fff" stroke="#fefefe" viewBox="0 0 25 16" width="25" height="16" role="presentation" focusable="false"><use xlink:href="/assets/images/defs.svg#icon-email"></use><title>email</title></svg></span>Exclusive Access</a>
            </li>`);
        }
