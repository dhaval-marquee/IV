function loadTestCode() {
    ...
    if (!bodyEle.classList.contains('homewarrantyreviews_test')) {
		...
		newElement.innerHTML = `<a href="https://www.homewarrantyreviews.com/home-warranty-price-estimate/">Ready to get started? Get protected with your free quote</a>
		...
		setTimeout(() => {
			document.querySelectorAll('.homewarrantyreviews_test form input[name="btn_location"]').forEach(function(inputValue){
		...
		setTimeout(() => {
			document.querySelectorAll('.homewarrantyreviews_test form input[name="pe_btn_location"]').forEach(function(inputValue){
				var getVal = inputValue.getAttribute('value');
				inputValue.setAttribute('value',getVal+'_V1');
			});
		}, 2000);

		document.querySelectorAll(".free_quote .Cta_btn").forEach((qsitem) => {
			qsitem.addEventListener("click", function (e) {
				setTimeout(() => {
					document.querySelectorAll('.homewarrantyreviews_test form input[name="pe_btn_location"]').forEach(function(inputValue){
						var getVal = inputValue.getAttribute('value');
