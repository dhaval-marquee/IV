function loadTestCode() {
    ...
    if (!bodyEle.classList.contains('color_iv')) {
        ...
        var getInterval = setInterval(() => {
            if($('.color_iv .popular-colors .color-tile').length > 4 && $('.color_iv .popular-colors .color-tile').find('span.color-tile__swatch').attr('style') != undefined) {
                clearInterval(getInterval)

                var getColorCode = $('.color_iv .popular-colors').html();
                TotalColor = $('.all-colors-container .color-tile').length;

                $('.color_iv .mobile-color-button').before(`<div class="popular-color-iv">`+getColorCode+`</div>`);
