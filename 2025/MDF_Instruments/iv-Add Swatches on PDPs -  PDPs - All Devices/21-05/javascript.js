function loadSwatches() {
    ...
    if (!jQuery('body').hasClass('swatches_iv')) {
        ...
        const swatchesWrapper = jQuery('<div class="product-swatches"><div class="imagesWrapper"></div></div>');

        matchedSwatches.forEach(swatch => {
            const anchor = jQuery(`<a href="${swatch.url}"></a>`);
            const img = jQuery(`<img src="${swatch.img.src}" alt="${swatch.img.alt}">`);
            anchor.append(img);
            swatchesWrapper.find('.imagesWrapper').append(anchor);
        });
