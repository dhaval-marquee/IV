function loadTestCode() {
    ...
    // Clone appendSection into each complementary products container
    const complementarySections = document.querySelectorAll(
        '.cart-drawer__complementary-products.complementary-products'
    );

    complementarySections.forEach((section) => {
        if (appendSection) {
            // Avoid duplicate clone
            if (!section.querySelector('.iv-footer-clone')) {
                const clone = appendSection.cloneNode(true);
                clone.classList.add('iv-footer-clone'); // marker class
