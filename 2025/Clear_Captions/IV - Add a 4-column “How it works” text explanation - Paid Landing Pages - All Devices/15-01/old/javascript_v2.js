    ...
    if (!jQuery('body').hasClass('howItWorkIV')) {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                if (NumberforDesktop.length && NumberforMobile.length) {
                    const NumberforDesktopHTML = NumberforDesktop.prop('outerText');
                    const NumberforMobileHTML = NumberforMobile.prop('outerText');
                    jQuery('#dynamicNumber').eq(0).append(
                        `<span class="for-desktop">${NumberforDesktopHTML}</span><span class="for-mobile">${NumberforMobileHTML}</span>`
                    );
                }
            },
