function loadTestCode() {
    ...
    if (!jQuery('body').hasClass('howItWorkIV')) {
        ...
        var text = jQuery('h4.fusion-title-heading').eq(2).text();
        var contactNumber = text.match(/\d{3}-\d{3}-\d{4}/)[0];
        console.log(contactNumber); // Output: 866-949-4749

        var textNumber = jQuery('h4.fusion-title-heading').eq(1).text();
        var phoneNumberGet = textNumber.match(/\d{3}-\d{3}-\d{4}/)[0];
        console.log(phoneNumberGet); // Output: 866-949-4749
                    ...
                    <div class="howItWorksAccordion">
                        <div class="accordion-item active">
                            <div class="accordion-header">
                                ...
                                <h3>1. Complete our form or call <em class="title-italic" id="phoneNumber"><span class="only-desktop">${contactNumber}</span><span class="only-mobile">${phoneNumberGet}</span></em></h3>
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                // jQuery('#phoneNumber').eq(0).append(
                //     `<span class="only-desktop">${contactNumber}</span><span class="only-mobile">${contactNumber}</span>`
                // );
            },
