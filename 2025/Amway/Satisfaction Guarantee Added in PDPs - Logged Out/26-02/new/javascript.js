function loadTestCode() {
    if (!document.querySelector('body').classList.contains('iv-popup')) {
        ...
        if (!document.querySelector('.iv-learn-content')) {
            document.querySelector('.iv-popup .js-product-wrapper .amw-panel .AddToCart-ShareAction').insertAdjacentHTML('afterend', learnMoreContent);
        }

        const learnMoreContentMobile = `<div class="iv-learn-content iv-mobile-learn-content">
            <p>Buy with the confidence of our <br/><strong>Satisfaction Guarantee</strong></p>
            <a class="guarantee-cta" href="#">Learn More</a>
        </div>`;

        if (!document.querySelector('.iv-mobile-learn-content')) {
            document.querySelector('.iv-popup .js-product-wrapper .amw-panel .AddToCart-AddToCartAction').insertAdjacentHTML('beforeend', learnMoreContentMobile);
        }
    ...
    if(guaranteePopup){
        document.querySelector('.guarantee-cta').addEventListener('click', function(){
            guaranteePopup.style.display = 'block';
        });
        document.querySelector('.guarantee-popup-closed').addEventListener('click', function(){
