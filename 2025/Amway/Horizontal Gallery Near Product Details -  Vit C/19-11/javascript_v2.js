function loadTestCode() {
    if (!document.body.classList.contains('iv-pd')) {
        ...
        document.body.classList.add('iv-pd_v2');
        ...
        `;
        document.querySelector('.iv-pd_v2 #productPageAccordion').insertAdjacentElement("afterend", carousel);
        ...
        document.querySelectorAll('.iv-image-carousel > div').forEach((imgDiv, index) => {
            imgDiv.addEventListener('click', () => {
                const items = document.querySelectorAll('.amw-gallery-carousel--primary .amw-gallery-carousel__img-container.js-carousel-item');
                if (items[index]) {
                    items[index].click();
                }
            });
