function loadTestCode() {
    if (!document.body.classList.contains('iv-dropdown')) {
        ...
        const menuData = [{
                ...
                submenus: [{
                    ...
                    },
                    {
                        ...
                        children: [{
                                name: "Shop All Flying Insect Traps",
                    ...
                    },
                    {
                        ...
                        children: [{
                                name: "Shop All Crawling Insect Traps",
                ...
                ${m.slider ? `
                    ...
                    <div class="iv-menu-tabs">
                        ${menuData.map((m, i) =>
                            ...
                            ? `<a href="${m.link}" class="iv-tab iv-direct-link" target="_blank">${m.title}</a>`
        ...
        const searchIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 30 30" fill="none">
        <path d="M15.7955 15.8111L21 21M18 10.5C18 14.6421 14.6421 18 10.5 18C6.35786 18 3 14.6421 3 10.5C3 6.35786 6.35786 3 10.5 3C14.6421 3 18 6.35786 18 10.5Z" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>`;

        var searchContent = document.querySelector('.search-modal__content');
        var menuContent = document.querySelector('.iv-menu-content');

        if (searchContent && menuContent) {
            menuContent.insertBefore(searchContent, menuContent.firstChild);
            var searchInput = document.querySelector('.iv-menu-content #Search-In-Modal');
            if (searchInput) {
                searchInput.setAttribute('placeholder', 'What are you looking for today?');
            }
            document.querySelector('.iv-menu-content .search .search__button').insertAdjacentHTML('afterbegin', searchIcon);
        }
        ...
        document.addEventListener("click", (e) => {
            ...
            const clickedTab = e.target.closest(".iv-tab");
            if (!clickedTab) return;

            if (clickedTab.classList.contains("iv-direct-link") || e.target.closest(".iv-direct-link")) return;

            const clickedPanel = clickedTab.closest(".iv-tab-panel");

            if (clickedTab.classList.contains("iv-active")) {
                const idx = clickedTab.dataset.tab;
                const currentPanel = document.querySelector(`.iv-tab-panel[data-content="${idx}"]`);
                if (currentPanel && currentPanel.classList.contains("iv-show")) return;
            }

            const idx = clickedTab.dataset.tab;
            const targetPanel = document.querySelector(`.iv-tab-panel[data-content="${idx}"]`);
            if (!targetPanel) return;

            document.querySelectorAll(".iv-tab-panel").forEach(p => p.classList.remove("iv-show"));
            targetPanel.classList.add("iv-show");

            document.querySelectorAll(".iv-tab-panel").forEach(panel => {
                const menu = panel.querySelector(".iv-menu-tabs");
                if (!menu) return;
                menu.querySelectorAll(".iv-tab").forEach(t => t.classList.remove("iv-active"));
            });

            const tabInTargetPanel = targetPanel.querySelector(`.iv-menu-tabs .iv-tab[data-tab="${idx}"]`);
            if (tabInTargetPanel) {
                tabInTargetPanel.classList.add("iv-active");
            } else {
                clickedTab.classList.add("iv-active");
            }
        });
    }
}
