...
function loadTestCode() {
    if (!document.body.classList.contains('iv-PCP')) {
        ...
        // Shop by Pest and Environment section needs to be removed (/collections/rats-mice)
        let cachedTargetSection = null;
        let originalParent = null;
        let originalNext = null;
        function moveSection() {
            if (!location.href.includes('/collections/rats-mice')) return;
            if (!cachedTargetSection) {
                cachedTargetSection = document.querySelector('.iv-PCP .section-collection-list + section + section');
                if (!cachedTargetSection) return;
                originalParent = cachedTargetSection.parentNode;
                originalNext = cachedTargetSection.nextElementSibling;
            }
            if (cachedTargetSection.parentNode) {cachedTargetSection.parentNode.removeChild(cachedTargetSection);}
            if (window.innerWidth < 1024) {
                if (originalNext) {
                    originalParent.insertBefore(cachedTargetSection, originalNext);
                } else {
                    originalParent.appendChild(cachedTargetSection);
                }

            } else {
                const productGrids = document.querySelector('.iv-PCP #MainContent > div > div > facet-filters-form.facets > form#FacetSortForm');
                if (productGrids) {productGrids.insertBefore(cachedTargetSection, productGrids.firstChild);}
            }
        }
        moveSection();
        ...
        document.querySelectorAll('.quick-add-modal').forEach(modal => {
            ...
            // Close when clicking the modal itself (outside content)
            modal.addEventListener('click', (e) => {
                // only close if clicked the background, NOT the inner content
                if (e.target === modal) {
                    modal.removeAttribute('open');
                }
            });
        ...
        // ############# On Mobile Filter #####################
        const mobileFilterBtn = document.querySelector(".iv-mobile-filter");

        if (!mobileFilterBtn) return;

        mobileFilterBtn.addEventListener("click", () => {

            const allDetails = document.querySelectorAll("#FacetFiltersForm details");
            const sortOptions = document.querySelector(".iv-custom-sort-options");
            const sortHeader = document.querySelector(".iv-sort-header");
            ...
            // Close any open filter when mobile filter button is clicked
            closeAllFilters();
            ...
            summary.addEventListener("click", event => {
                ...
                if (sortOptions && window.getComputedStyle(sortOptions).display === "flex") {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (e.target.id == "buyNow") {
                e.preventDefault();
                alert("Clicked");
                e.nextElementSibling.querySelector('.cro-use-quick-add').click();
            }
        ...
        document.querySelectorAll('.iv-PCP .product-grid-container .collection .card-wrapper').forEach(function(card) {
            if (card.querySelector('.buyNow')) return;
            const quickAdd = card.querySelector('.quick-add');
            if (!quickAdd) return;
            quickAdd.insertAdjacentHTML(
                'beforebegin',
                `<a href="javascript:;" class="buyNow" id="buyNow">Buy Now</a>`
            );
        });
        
        // window.addEventListener("click", function (btn) {
        //     if (btn.target.id == "buyNow") {
        //         e.preventDefault();
        //         alert("Clicked");
        //         btn.nextElementSibling.querySelector('.cro-use-quick-add product-form').click();
        //     }
        // });
