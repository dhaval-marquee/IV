  ...
  script.onload = function () {
    ...
    jQuery(function ($) {
      if (!$('body').hasClass('iv-contact')) {
        ...
        $('.iv-contact .lp-pom-form.has-axis form').append('<button id="ivContinueCta" class="lp-element lp-pom-button" type="button"><span class="label">Continue</span></button>');
      }
      ...
      var getHeight = setInterval(() => {
        if ($('.lp-pom-form form').length > 0) {
          ...
          var collapse_page_load = function() {
          ...
          var collapse_page_click = function() {
            var currFormHeight = $('.lp-pom-form').height();
            var diff = form_wrap_height - currFormHeight + 200;
            if (form_wrap_height > currFormHeight) {
              form_wrapper.height(currFormHeight);
            }
            form_block.height(form_block.height() + diff);
          ...
          collapse_page_load();
          $('.iv-contact button[type=button]').on('click', function () {
            ...
            if (firstName && lastName && emailValid) {
                $('.iv-contact .lp-pom-form.has-axis form').addClass('show-all-fields');
                // contact Form Continue Button event
                window.VWO = window.VWO || [];
                VWO.event = VWO.event || function () {VWO.push(['event'].concat([].slice.call(arguments)))};
                VWO.event('contactFormContinueButton')
                collapse_page_click();
            } else {
              $('.iv-contact .lp-pom-form.has-axis form')[0].reportValidity();
            }
          });
