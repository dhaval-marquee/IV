function loadTestCode() {
    if (!document.querySelector('body').classList.contains('iv-sticky-button')) {
        ...
        let packageName = document.querySelector('.proudct-breadcrumb.d-xl-none ul li a').innerText.toLowerCase();
        packageName = packageName.replace(/packages$/, 'package');
        const productNameDiv = `<div class="sticky-product-name">${productName} - ${packageName.charAt(0).toUpperCase() + packageName.slice(1)}</div>`;
        ...
        const checkInterval = setInterval(adjustOffset, 2000);
