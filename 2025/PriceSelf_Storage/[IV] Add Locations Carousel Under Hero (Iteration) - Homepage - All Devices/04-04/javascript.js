                        ...
                        function(position) {
                            ...
                            $(document).ready(function () {
                                function initializeSlider() {
                                    const $slider = $('#MainContent_LocationRepeater');
                                    const screenWidth = $(window).width();
                                    const itemCount = $('.iv-location-section .dynamicContentWrapper #MainContent_LocationRepeater > span').length;
                                    const shouldInitializeSlider = ((screenWidth < 768) && itemCount > 1) || (itemCount > 3);

                                   
                            
                                    if (shouldInitializeSlider) {
                                        const totalSlides = $('#MainContent_LocationRepeater .show').length;
                                        ...
                                        function updateProgressBar(currentSlide) {
                                            const visibleSlides = $slider.slick('slickGetOption', 'slidesToShow');
                                            const progress = ((currentSlide) / (totalSlides - visibleSlides)) * 70;
                                            const finalProgress = 30 + progress;
                                            $progressBar.css({
                                                'width': finalProgress + '%',
                                ...
                                let previousOrientation = isPortrait() ? 'portrait' : 'landscape';
                                  
                                window.addEventListener('orientationchange', () => {
                                    const currentOrientation = isPortrait() ? 'portrait' : 'landscape';
                                   
                                    if (previousOrientation !== currentOrientation) {
                                        if ($('#MainContent_LocationRepeater').hasClass('slick-initialized')) {
                                            $('#MainContent_LocationRepeater').slick('unslick');
                                        }
                                      setTimeout(() => {
                                        initializeSlider();
                                      }, 300);
                                ...
                                // Run on resize (with debounce to improve performance)
                                // let resizeTimeout;
