...
function loadTestCode() {
    if (!document.querySelector('body').classList.contains('iv-web-rollout-question')) {
        ...
        const buttons = document.querySelectorAll('.pss-button-box a, .pss-item-card-right-button a');
        if (buttons.length > 0) {
            buttons.forEach((btn) => {
                btn.addEventListener('click', (e) => {
                    e.preventDefault();
                    document.body.classList.add('iv-pss-active');
                    e.stopPropagation();
                });
            });
            ...
            document.addEventListener('click', (e) => {
                if (!e.target.closest('.pss-button-box a, .pss-item-card-right-button a')) {
                    document.body.classList.remove('iv-pss-active');
                }
            });
        }
...
const checkConditionWeb = setInterval(function() {
    if (document.querySelector('.pss-button-box') || document.querySelector('.pss-item-card-right-button')) {
        clearInterval(checkConditionWeb);
        loadTestCode();
        defultClick()
    }
}, 50);
