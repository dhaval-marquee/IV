function loadTestCode() {
    if (!document.querySelector('body').classList.contains('iv-guide-popup')) {
                    ...
                    <div class="storage-guide-popup-content">
                        <div class="storage-guide-popup-title">
                            ...
                            <div class="iv-closest-size">
                                <strong></strong> is the closest to your selected size
                            </div>
        ...
        // Descriptions and images - exact match with button text
        const contentData = {
            'Locker': {
        ...
        // Automatically trigger click on the first size tab in the group
        ...
        ivPopupTargets.forEach((popupBtn) => {
            ...
            popupBtn.addEventListener('click', () => {
                ...
                if (popup) {
                    ...
                    if (!document.querySelector('.iv-guide-popup-overflow')) {
                ...
                if (unitItem?.classList.contains('pss-unit-list__item')) {
                    const prevElement = unitItem.previousElementSibling;
                    const sizeElement = prevElement?.querySelector('.sizes') || unitItem.querySelector('.sizes');
                    if (sizeElement) {
                        sizeText = sizeElement.innerText.trim();
                    }    
                } else if (unitItem?.classList.contains('pss-visible-mobile-only3')) {
                    const prevElement = unitItem.previousElementSibling;
                    const titleElement = prevElement?.querySelector('.pss-item-card-holder-left-title') || unitItem.querySelector('.pss-item-card-holder-left-title');
                    if (titleElement) {
                        sizeText = titleElement.innerText.trim();
                    }
                }

                const sizeOnly = sizeText?.match(/^\d+(?:\.\d+)?\s?[Xx×]\s?\d+(?:\.\d+)?/);
                if (sizeOnly) {
                    const outputTarget = document.querySelector('.iv-closest-size strong');
                    if (outputTarget) {
                        outputTarget.textContent = sizeOnly[0];
                    }
                }
            });
        });
