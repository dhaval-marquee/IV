    ...
    if (!document.querySelector('body').classList.contains('iv-guide-popup')) {
        ...
        sizeTabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                if (outputTarget && closestSizeContainer) {
                    const closestSize = extractSize(outputTarget.textContent.trim());
                    const activeSize = extractSize(key);

                    if (closestSize && activeSize && (closestSize === activeSize) ||  outputTarget.textContent.trim().toLowerCase() === 'n/a') {
