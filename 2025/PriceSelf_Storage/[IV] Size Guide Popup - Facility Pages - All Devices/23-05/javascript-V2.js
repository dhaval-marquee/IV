    ...
    if (!document.querySelector('body').classList.contains('iv-guide-popup')) {
        ...
        ivPopupTargets.forEach((popupBtn) => {
            ...
            popupBtn.addEventListener('click', () => {
                ...
                let normalizedSizeText = sizeTextForTab ? sizeTextForTab.replace(/\s+/g, '').replace(/[^\dx]/g, '').toLowerCase() : '';

                // Try exact match from closestSizeMap
                for (const [key, value] of Object.entries(closestSizeMap)) {
                    const normalizedKey = key.toLowerCase().replace(/\s+/g, '');
                    if (normalizedSizeText === normalizedKey.replace(/[^\dx]/g, '')) {
                        matchedValue = value;
                        hasMatch = true;
                        break;
                    }
                ...
                // Show closest size box only for valid sizes
                if (outputTarget) {
                    outputTarget.textContent = matchedValue;

                    if (validMatches.includes(normalizedSizeText)) {
                ...
                // If Vaule NULL then none this bar
                document.querySelectorAll('.iv-closest-size').forEach(sizeNull => {
                    const closestSizeValue = sizeNull.querySelector('strong')?.innerText.trim();
                    if (closestSizeValue === 'n/a' && !sizeNull.classList.contains('iv-closest-size-null')) {
                        sizeNull.style.display = 'none';
                    }
                });
