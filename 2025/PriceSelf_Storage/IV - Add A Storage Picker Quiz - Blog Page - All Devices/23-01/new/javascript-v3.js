function blogPageV3() {
    if (!document.querySelector('body').classList.contains('iv-quiz-form')) {
        ...
        if (!document.querySelector('.storage-section') && document.querySelector('.iv-quiz-form #content-area .post-content')) {
            if (pathName.includes('/blog/what-size-storage-unit-for-a-car-is-best') || pathName.includes('/blog/how-much-does-it-cost-to-store-a-car')) {
                document.body.classList.add('iv-middle')
                document.querySelector('figure').insertAdjacentHTML('afterend', storageBoxSection)
            } else if (pathName.includes('/blog/how-to-store-shoes-long-term')) {
                document.querySelectorAll('h2').forEach((step) => {
                    if (step.innerText.toLowerCase() === "step 4: bag your shoes".toLowerCase()) {
                        step.insertAdjacentHTML('beforebegin', storageBoxSection)
                    }
                });
            } else {
                const headingsH2 = document.querySelectorAll('.post-content h2');
                const headingsH3 = document.querySelectorAll('.post-content h3');
                if (headingsH3.length >= 4) {
                    headingsH3[3].insertAdjacentHTML('beforebegin', storageBoxSection);
                } else if (headingsH2.length >= 4) {
                    headingsH2[3].insertAdjacentHTML('beforebegin', storageBoxSection);
                } else if (headingsH3.length < 4 && headingsH3.length !== 0) {
                    headingsH3[1].insertAdjacentHTML('beforebegin', storageBoxSection);
                } else if (headingsH2.length < 4 && headingsH2.length !== 0) {
                    headingsH2[1].insertAdjacentHTML('beforebegin', storageBoxSection);
                }
            }
        ...
        if (!document.querySelector('.quiz-model')) {
            ...
            document.querySelectorAll('.storage-box').forEach(box => {
                box.addEventListener('click', function() {
                    ...
                    const vehicleQuiz = localStorage.getItem('model-title');
                    const displayStyle = vehicleQuiz === 'Vehicle' ? 'none' : '';
                    document.querySelectorAll('.step-2 .quiz-question, .step-2 .quiz-storage, .step-2 .toggle-section').forEach(hideV => hideV.style.display = displayStyle);
                    ...
                    if (modelTitle === 'Self Storage Solutions') {
                        ...
                        localStorage.setItem('model-title', 'Self Storag');
            ...
            document.querySelector('.closed-button').addEventListener('click', function() {
                ...
                if (document.querySelector('.select-error').style.display = "block") {
                    document.querySelector('.select-error').style.display = "none";
                }
            ...
            if (document.querySelector('.next-cta')) {
                document.querySelector('.next-cta').addEventListener('click', function() {
        ...
        function triggerSpecificTabsBasedOnSessionStorage(key, containerSelector) {
         ...
         // click search 
         const locationSearch = localStorage.getItem('userLocation');
         if (locationSearch) {
            const loaderCode = `<div id="pnlSpinner" class="pss-loader iv-pss-loader">
                            <div class="pss-loader__dual-ring"></div>
                        </div>`;
            if(window.location.href.includes('/locations') || window.location.href.includes('/locations/')){
                const body = document.body
                body.insertAdjacentHTML('afterbegin', loaderCode);
                body.classList.add('iv-locations-page');
            }

             const inputField = document.querySelector('.pss-search-form .pss-locations__search-input input');
             if (inputField) {
                 inputField.value = locationSearch;
 
                 const searchButton = document.querySelector('.pss-locations__search-action input');
                 if (searchButton) {
                     searchButton.click();
                     sessionStorage.setItem('clickSearch', 'true');
                     localStorage.removeItem('userLocation');
                 }
             }
         }
         const clickSearch = sessionStorage.getItem('clickSearch');
 
         if (!locationSearch && clickSearch) {
             if (document.querySelector('span .pss-card-list.show-flex')) {
                 let showCard = document.querySelectorAll('span .pss-card-list.show-flex');
                 if (showCard.length > 0) {
                     let firstCard = showCard[0];
                     let firstLink = firstCard.querySelector('a');
 
                     if (firstLink) {
                         firstLink.click();
                         sessionStorage.removeItem('clickSearch')
                     }
                 }
             }
         }
