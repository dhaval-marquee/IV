function blogPageV1() {
    if (!document.querySelector('body').classList.contains('iv-quiz-form')) {
            ...
            if (document.querySelector('.next-cta')) {
                document.querySelector('.next-cta').addEventListener('click', function() {
                    ...
                    if (selectedOption) {
                        ...
                        selectError.style.display = 'block'; // Show the error
        ...
        // Function to handle vehicle-specific logic
        function handleVehicleClick(key) {
            if (modelTitle === 'Vehicle') {
                const filtersRow = document.querySelector(".locationContent .pss-filters-row__advanced-filters");
                const filtersOpen = document.querySelector('#toggleFacilitiesFiltersBox')
                const filtersRowTabs = setInterval(() => {
                    if (filtersRow && filtersOpen) {
                        clearInterval(filtersRowTabs);

                        const container = document.querySelector(".locationContent .pss-filters-row");
                        const buttons = container.querySelectorAll("button");
                        buttons.forEach(button => {
                            const buttonText = button.textContent.trim();
                            if (buttonText === 'ALL') {
                        ...
                        filtersRow.click();
                        const filtersRowInputTime = setInterval (() => {
                            
                            const filtersRowInput = filtersOpen.querySelectorAll('input');
                            if (filtersRowInput.length > 0) {
                                clearInterval(filtersRowInputTime)
                                filtersRowInput.forEach(filterOpenInput => {
                                    const filterOpenInputValue = filterOpenInput.value.trim();
                                    if (filterOpenInputValue === 'Vehicle Storage') {
                                        filterOpenInput.click();
                                    }
                                    if (filterOpenInputValue === 'RV/Boat Outdoor Parking') {
                                        filterOpenInput.click();
                                    }
                                })
                            }
